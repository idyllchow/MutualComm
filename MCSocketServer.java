...
import com.idyll.mutualcomm.utils.DeviceUtil;
...
//            int socketID = 0;//Android（SocketClient）客户机的唯一标志，每个socketID表示一个Android客户机
            ...
            while (isStartServer) {
                ...
                LogUtil.defaultLog("-----socketID imei == " + DeviceUtil.getIMEI());
                SocketThread thread = new SocketThread(socket, DeviceUtil.getIMEI());
                thread.start();
                //将该线程添加到线程队列
    ...
    public class SocketThread extends Thread {
        ...
        private ArrayList<String> socketIDList = new ArrayList<>();
//        private StringBuffer socketIDBuffer = new StringBuffer();
        private boolean isAdd = false;

        Set socketIDSet = new HashSet();
        public SocketThread(Socket socket, String imsi) {
            socketID = imsi;
            this.socket = socket;
...
//            socketIDBuffer.append(socketID + "&");
            ...
            LogUtil.defaultLog("----set socketID " + SponiaSpUtil.getDefaultSpValue(SpCode.SOCKET_ID));
//            SponiaSpUtil.setDefaultSpValue(SpCode.SOCKET_ID, SponiaSpUtil.getDefaultSpValue(SpCode.SOCKET_ID).toString() + socketID + "&");
            ...
            try {
                ...
                while (isStartServer) {
                    ...
                    if (reader.ready()) {
                        ...
                        mMsgList.add(msg);
                        LogUtil.defaultLog("收到一条消息：" + json.getString("msg") + " >>>> to socketID:" + json.getString("to"));
